...
class IBBroker:
    ...
    def connect(self):
        ...
        log.info("IB connecting to %s:%s with clientId %s", self._host, self._port, client_id)
        try:
            self._ib.connect(self._host, self._port, clientId=client_id, timeout=15, readonly=False)
            self._last_error = None
